...
function normalizeState(state) {
  ...
  const seen = new Map(defaultCategories.map((category) => [category.id, category]));
  ...
  if (Array.isArray(state.categories)) {
    for (const category of state.categories) {
      ...
      seen.set(category.id, {
        ...
        removable: category.removable !== false
      });
    }
  }

  normalized.categories = Array.from(seen.values()).map((category) => ({
    ...category,
    removable: defaultCategories.some((item) => item.id === category.id) ? false : category.removable
  }));
...
function normalizeMonthData(monthData, categories, monthKey = currentMonthKey()) {
  const budgets = {};
  const transactions = Array.isArray(monthData?.transactions) ? monthData.transactions : [];

  for (const category of categories) {
    const rawBudget = monthData?.budgets?.[category.id];
    budgets[category.id] = sanitizeNumber(rawBudget);
  }
...
function renderSummary(monthData) {
  ...
  const summaryItems = [
    ...
    },
    {
      ...
      note: `${formatMoney(metrics.remainingBudget)} remaining`
    },
    {
...
function renderInsights(monthData) {
  ...
  for (const item of metrics.categoryBreakdown) {
    ...
    fragment.querySelector(".progress-values").textContent = `${formatMoney(item.spent)} / ${formatMoney(item.budget)}`;
...
function populateTransactionCategories() {
  ...
  const options = transactionTypeSelect.value === "income"
    ? extraIncomeCategories.concat(appState.categories.map((category) => ({ id: category.id, name: category.name })))
...
function handleAddCategory(event) {
  ...
  if (appState.categories.some((category) => category.id === id)) {
...
function handleTransactionDelete(event) {
  ...
  const monthData = getCurrentMonthData();
  monthData.transactions = monthData.transactions.filter((transaction) => transaction.id !== button.dataset.transactionId);
  render();
}
